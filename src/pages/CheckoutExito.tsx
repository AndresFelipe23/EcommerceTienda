...
export default function CheckoutExito() {
  ...
  useEffect(() => {
    const procesarPagoExitoso = async () => {
      // Si ya hay un pedidoId, solo cargarlo
      if (pedidoId) {
        try {
      ...
      // Si no hay pedidoId, verificar si hay datos temporales de Mercado Pago
      const pedidoTemporal = localStorage.getItem('pedidoTemporal');
      if (!pedidoTemporal) {
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'No se encontraron datos del pedido',
      ...
      try {
        ...
        const datos = JSON.parse(pedidoTemporal);
        
        // Crear el pedido ahora que el pago fue exitoso
        const response = await pedidoService.crearDesdeCarrito({
          ...
          direccionFacturacion: datos.direccionFacturacion,
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Error al crear el pedido. Por favor, contacta al soporte.',
          confirmButtonText: 'Entendido',
        });
      } finally {
        setIsLoading(false);
    ...
    procesarPagoExitoso();
  }, [pedidoId, navigate, carrito, refreshCart]);

  if (isLoading || isCreatingPedido) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 text-lg">
            {isCreatingPedido ? 'Creando tu pedido...' : 'Verificando pago...'}
          </p>
        </div>
      </div>
    );
